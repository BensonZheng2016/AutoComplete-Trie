...
private:
	CTrie_Node* root;
public:
	void Insert(string& key, int point = 0, int counter = 0);
...
//counterPress chua hoan thien!!!
vector<int> counterTemp;//test counterPress
void SortStringDepenCounter(vector<string>& collect)
{
	for (int i = 0; i < counterTemp.size() - 1; i++)
		for (int j = i + 1; j < counterTemp.size(); j++)
		{
		if (counterTemp[i] < counterTemp[j])
		{
			//swap
			int temp = counterTemp[i];
			counterTemp[i] = counterTemp[j];
			counterTemp[j] = temp;
			string tmp = collect[i];
			collect[i] = collect[j];
			collect[j] = tmp;
		}
		}
...
vector<string> FindWord(CTrie_Node* subtree, vector<string>& collect){
	for (int i = 0; i < subtree->children.size(); i++)
	{
		...
		{
			counterTemp.push_back(subtree->children[i]->counterPress);
			collect.push_back(subtree->children[i]->word);
			SortStringDepenCounter(collect);
...
vector<string> CTrie::FindWordCont(char key, CTrie_Node* &pos)
{
	...
	if (subtree != NULL && subtree->CheckLeaf())//
	{
		collect.push_back(subtree->word);//
		counterTemp.push_back(subtree->counterPress);
	}
	//neu ko co thi thoat
	if (subtree == NULL)
	{
...
void Export(CTrie_Node* subtree, fstream& fo)
{
	...
	if (subtree->CheckLeaf())
	{
		fo << subtree->word << endl << subtree->prior << endl << subtree->counterPress << endl;
		countWordEx++;
...
void CTrie::ImportDic(string fileIn)
{
	...
	while (!tmp.empty())
	{
		countWordIm++;
		getline(fi, tmpPoint);
		getline(fi, tmpCounter);
		this->Insert(tmp, stoi(tmpPoint), stoi(tmpCounter));//
		getline(fi, tmp);
	}
...
void CTrie::Insert(string& str, int point, int counter){
	...
	for (int i = 0; i < str.length(); i++){
		...
		if (i == (str.length() - 1))
		{
			...
			current->SetWord(str);
			//if (counter >= 1)
			current->counterPress += counter;//counterPress
		}
	}
